"""
Testes de integração do ClassificationAgent contra datasets anotados.
Requer Ollama rodando localmente com qwen2.5:7b (ver tests/integration/conftest.py).
"""
...
def agent(classifier_model, dataset):
    return ClassificationAgent(model=classifier_model, nfr_categories=dataset.nfr_categories)


@pytest.fixture(scope="module")
def classified_sample(agent, dataset_sample):
    return agent.classify_batch(dataset_sample)


class TestClassificador:
    def test_todos_classificados(self, classified_sample, dataset_sample):
        assert len(classified_sample) == len(dataset_sample)

    def test_f1_macro_minimo(self, classified_sample, labeled_sample):
        """F1 macro mínimo: 0.60 para Ollama local."""
        y_true = [r.metadata["label_type"] for r in labeled_sample]
        y_pred = [r.requirement_type.value for r in classified_sample]
        f1 = f1_score(y_true, y_pred, average="macro", zero_division=0)
        print(f"\nF1 Macro: {f1:.4f}")
        assert f1 >= 0.60

    def test_acuracia_geral(self, classified_sample, labeled_sample):
        y_true = [r.metadata["label_type"] for r in labeled_sample]
        y_pred = [r.requirement_type.value for r in classified_sample]
    ...
    def test_report_por_categoria(self, classified_sample, labeled_sample):
        y_true = [r.metadata["label_type"] for r in labeled_sample]
        y_pred = [r.requirement_type.value for r in classified_sample]
...
class TestClassificadorIdioma:
    def test_divergencia_en_vs_pt(self, agent, dataset_sample):
